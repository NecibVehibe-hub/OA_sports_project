...
function Navbar(){
    ...
    return(
        <div className='w-screen top-0 left-0 center fixed z-50 flex justify-between flex-col   lg:hidden'>
            ...
            <nav className={`${menuIsOpen ? "":"hidden"} w-screen bg-blackBg  h-[80%] overflow-y-scroll flex items-center px-4 pb-3 flex-col pt-4`}>
                <ul id="menu" className='w-screen p-4 h-114.75 flex flex-col  justify-between '>
                ...
                <span className='w-full mt-6 flex justify-between'><li className='font-bold list-none leading-none text-navFont'>{t("navbar.Language")}</li><button onClick={()=>setState(!stateDown)} className='flex gap-1 text-navFont'><img src="" alt="" /><span className='text-mainYellow'>{language}</span>{stateDown ? <ChevronUpIcon className='w-6 h-6 text-mainYellow'></ChevronUpIcon>:<ChevronDownIcon  className='w-6 h-6 text-mainYellow'></ChevronDownIcon>}</button></span>
                <hr className='h-4' />
                <div className={`${stateDown? "hidden":""} w-full p-4 overflow-x-hidden  flex flex-col gap-3 h-43 overflow-y-auto`}>
                    {languages.map((language)=>(<li onClick={()=>{setLanguage(language.name);i18n.changeLanguage(language.name)}} className='text-white list-none'>{language.label}</li>))}
                </div>
            </nav>
        </div>
    )
