...
function Card({title,text,img,isOpen}){
    return(
        <div style={{ backgroundImage: `url(${img})` }} className={`p-2.75  h-full rounded-2xl flex items-end bg-cover bg-position-[center_40%]`}>
        {isOpen && (
            <div id="textDiv" className="h-36 md:min-w-75  rounded-2xl flex flex-col  p-3 backdrop-blur-md bg-white/1 border border-x-0  border-b-white/20 border-t-white/20">
...
function WhatOASportsDo(){
    ...
    return(
        <div className="w-full h-215 md:h-100 bg-bg py-6 px-4 flex flex-col gap-3 3xl:w-360 3xl:py-12 3xl:px-24 3xl:h-148.5 3xl:gap-8">
            ...
            <div className="w-full h-189 flex flex-col md:w-full md:flex-row gap-4 3xl:gap-3.5">
                {cardsList.map((card,index)=>(
                <div key={index} className={`w-full 3xl:h-100  ${IsOpen!==index ?"md:h-full":""}  ${IsOpen===index ? ("min-h-80 md:w-100"):"h-25"}`} onClick={()=>setIsOpen(index)} >
